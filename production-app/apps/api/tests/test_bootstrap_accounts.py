    ...
    manifest.write_text(
        json.dumps(
            [
                ...
                {"username": "processor1", "email": "processor1@pimascor.com", "display_name": "Processor 1", "role": "REQUESTER"},
...
def test_bootstrap_rejects_section_one_incomplete_processor_email(tmp_path, monkeypatch):
    manifest = tmp_path / "accounts.json"
    manifest.write_text(
        json.dumps([{"username": "processor1", "email": "processor1@pimascor", "display_name": "Processor 1", "role": "REQUESTER"}]),
        encoding="utf-8",
    )
    monkeypatch.setattr(bootstrap_accounts, "SessionLocal", TestingSession)
    monkeypatch.setattr(sys, "argv", ["bootstrap-accounts", "--manifest", str(manifest)])
