...
- Run both deploy scripts from the Mac and verify each performs transfer followed by
  automatic VPS activation, API health, Caddy validation, and public-route checks.
- Confirm new updates create no rollback image, previous web directory, backup
  timer, retention unit, database-dump unit, or restore helper.
- Confirm the production updater disables/removes only exact managed backup unit
  and helper paths; existing remote backup data and Podman secrets remain.
